...
class DFA {
  ...
  step(state: DFAState, charCode: number, anchor: number, context: number): DFAState | null {
    ...
    if (state.hasEmptyWidth) {
      ...
      cacheKey = charCode * 128 + (context & 0x3f) * 2 + (anchor === RE2Flags.UNANCHORED ? 0 : 1)
    ...
    } else if (anchor === RE2Flags.UNANCHORED && charCode <= Unicode.MAX_ASCII) {
  ...
  match(input: any, pos: number, anchor: number): boolean | null {
    ...
    while (i < endPos) {
      ...
      if (
        ...
        currentState =
          currentState.nextAscii[rune] || this.step(currentState, rune, anchor, context)
      } else {
  ...
  matchSet(input: any, pos: number, anchor: number): number[] | null {
    ...
    const checkMatch = (state: DFAState, currentPos: number, prevR: number): void => {
      if (state.isMatch) {
        if (anchor === RE2Flags.ANCHOR_BOTH) {
