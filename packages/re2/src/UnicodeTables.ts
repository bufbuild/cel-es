...
class UnicodeTables {
  ...
  private static _DELTA_CATEGORIES = /*#__PURE__*/ new LazyDecoder<Uint32Array>(
    {
      ...
      Mn: () =>
        decodeRanges(
          "3kCymhCymhCBDBvM-lB-lBBEBOECPBB47CkuQkuQBKBECB-lhBBB",
          false,
        ),
  ...
  private static _NEW_SCRIPTS =
    /*#__PURE__*/ new LazyDecoder<UnicodeRangeTable>({
  ...
  static readonly STABLE_CATEGORY_NAMES: ReadonlySet<string> = new Set([
    "C",
    "Cc",
    "Cf",
    "Cn",
  ...
  static readonly STABLE_SCRIPT_NAMES: ReadonlySet<string> = new Set([
    "Adlam",
    "Ahom",
  ...
  static readonly NEW_SCRIPT_NAMES: ReadonlySet<string> = new Set([
    "Garay",
    "Gurung_Khema",
    "Kirat_Rai",
    "Ol_Onal",
  ...
  static buildForProperty(name: string): UnicodeRangeTable | null {
    ...
    if (this.STABLE_CATEGORY_NAMES.has(name)) {
      kind = "category";
      pattern = `\\p{General_Category=${name}}`;
    } else if (this.STABLE_SCRIPT_NAMES.has(name)) {
      kind = "script";
    ...
    const delta =
      kind === "category"
        ? this._DELTA_CATEGORIES.get(name)
        : this._DELTA_SCRIPTS.get(name);
  ...
  static CATEGORIES = {
    has: (name: string): boolean =>
      UnicodeTables.STABLE_CATEGORY_NAMES.has(name),
    get: (name: string): UnicodeRangeTable | null =>
      UnicodeTables.buildForProperty(name),
  };

  static SCRIPTS = {
    has: (name: string): boolean =>
      UnicodeTables.STABLE_SCRIPT_NAMES.has(name) ||
      UnicodeTables.NEW_SCRIPT_NAMES.has(name),
    get: (name: string): UnicodeRangeTable | null =>
      UnicodeTables.buildForProperty(name),
  };
  ...
  static FOLD_CATEGORIES = {
    has: (name: string): boolean =>
      UnicodeTables.STABLE_CATEGORY_NAMES.has(name),
    get: (name: string): UnicodeRangeTable | null =>
      UnicodeTables.buildFoldOverlay(name),
  };
  ...
  static get Upper(): UnicodeRangeTable {
    return UnicodeTables.buildForProperty("Lu")!;
  }
}
