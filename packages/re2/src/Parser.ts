...
class Parser {
  ...
  static ERR_MISSING_REPEAT_ARGUMENT =
  ...
  static parseRepeat(t: StringIterator): number {
    ...
    if (
      ...
      throw new RE2JSSyntaxException(
        Parser.ERR_INVALID_REPEAT_SIZE,
        t.from(start),
      );
    }
  ...
  constructor(wholeRegexp: string, flags = 0) {
    ...
    this.numCap = 0;
    this.namedGroups = new Map<string, number>();
  ...
  repeat(
    ...
    if (n === 0) {
      throw new RE2JSSyntaxException(
        Parser.ERR_MISSING_REPEAT_ARGUMENT,
        t.from(beforePos),
      );
    }
  ...
  parsePerlFlags(t: StringIterator): void {
    ...
    if (t.lookingAt("(?P<") || t.lookingAt("(?<")) {
      ...
      if (!Parser.isValidCaptureName(name)) {
        // "(?P<name>"
        throw new RE2JSSyntaxException(
          Parser.ERR_INVALID_NAMED_CAPTURE,
          s.substring(0, end + 1),
        ); // "(?P<name>" or "(?<name>"
      }
