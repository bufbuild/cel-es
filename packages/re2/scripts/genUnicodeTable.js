...
import unicode15 from '@unicode/unicode-15.0.0'
...
const loadCodePoints = async (pkg, type, name) => {
  try {
    const { default: codePoints } = await import(`${pkg}/${type}/${name}/code-points.js`)
...
for (const [alias, name] of aliasesToNames.entries()) {
  ...
  const cp16 = await loadCodePoints('@unicode/unicode-16.0.0', 'General_Category', name)
  if (!cp16) continue
  const cp15 = await loadCodePoints('@unicode/unicode-15.0.0', 'General_Category', name)
  stableCategoryNames.push(alias)
  const delta = setDiff(cp16, cp15)
  if (delta.size > 0) {
    const ranges = computeRanges(delta)
    const enc = encodeRanges(ranges)
    deltaCatLines.push(`    '${alias}': () => decodeRanges('${enc.encoded}', ${enc.stride1}),`)
  }
}
...
for (const name of unicode16['Script']) {
  ...
  if (cp15) {
    ...
    if (delta.size > 0) {
      const ranges = computeRanges(delta)
      const enc = encodeRanges(ranges)
      deltaScrLines.push(`    '${name}': () => decodeRanges('${enc.encoded}', ${enc.stride1}),`)
    }
  } else {
...
const caseOrbitEntries = Array.from(reducedOrbit.entries()).sort((a, b) => a[0] - b[0])
let orbitEnc = ''
let curr = 0
...
const code = [
  ...
  '',
  'class UnicodeTables {',
  '  private static _CASE_ORBIT: Map<number, number> | null = null',
  '  static get CASE_ORBIT(): Map<number, number> {',
  '    if (!this._CASE_ORBIT) {',
  `      this._CASE_ORBIT = decodeOrbit('${orbitEnc}')`,
  '    }',
  ...
  '  private static _DELTA_CATEGORIES = /*#__PURE__*/ new LazyDecoder<Uint32Array>({',
  ...deltaCatLines,
  '  })',
  '',
  '  // Additions from Unicode 15.0 → 16.0 per stable script name.',
  '  private static _DELTA_SCRIPTS = /*#__PURE__*/ new LazyDecoder<Uint32Array>({',
  ...deltaScrLines,
  '  })',
  '',
  ...
  '  private static _NEW_SCRIPTS = /*#__PURE__*/ new LazyDecoder<UnicodeRangeTable>({',
  ...newScriptLines,
  '  })',
  '',
  `  static readonly STABLE_CATEGORY_NAMES: ReadonlySet<string> = new Set(${JSON.stringify(stableCategoryNames)})`,
  `  static readonly STABLE_SCRIPT_NAMES: ReadonlySet<string> = new Set(${JSON.stringify(stableScriptNames)})`,
  `  static readonly NEW_SCRIPT_NAMES: ReadonlySet<string> = new Set(${JSON.stringify(newScriptNames)})`,
  '',
  '  private static _sweepCache = new Map<string, UnicodeRangeTable>()',
  '  private static _foldCache = new Map<string, UnicodeRangeTable | null>()',
  '',
  ...
  '  static buildForProperty(name: string): UnicodeRangeTable | null {',
  '    if (this.NEW_SCRIPT_NAMES.has(name)) {',
  '      return this._NEW_SCRIPTS.get(name)',
  '    }',
  '    let kind: "category" | "script" | null = null',
  '    let pattern: string | null = null',
  '    if (this.STABLE_CATEGORY_NAMES.has(name)) { kind = "category"; pattern = `\\\\p{General_Category=${name}}` }',
  '    else if (this.STABLE_SCRIPT_NAMES.has(name)) { kind = "script"; pattern = `\\\\p{Script=${name}}` }',
  '    else return null',
  '',
  '    const cacheKey = `${kind}:${name}`',
  '    const cached = this._sweepCache.get(cacheKey)',
  '    if (cached) return cached',
  '',
  '    const base = sweepPlatform(pattern)',
  '    const delta = kind === "category" ? this._DELTA_CATEGORIES.get(name) : this._DELTA_SCRIPTS.get(name)',
  '    const merged = delta ? mergeRanges(base, delta) : base',
  '    const table = new UnicodeRangeTable(merged)',
  '    this._sweepCache.set(cacheKey, table)',
  '    return table',
  '  }',
  '',
  ...
  '  static buildFoldOverlay(name: string): UnicodeRangeTable | null {',
  '    const cached = this._foldCache.get(name)',
  '    if (cached !== undefined) return cached',
  '    const base = this.buildForProperty(name)',
  '    if (!base) {',
  '      this._foldCache.set(name, null)',
  '      return null',
  '    }',
  ...
  '      if (orbit.has(r)) return orbit.get(r)!',
  '      const s = String.fromCodePoint(r)',
  '      const lower = s.toLowerCase()',
  '      if (lower.length === s.length) {',
  '        const lowerCp = lower.codePointAt(0)!',
  ...
  '    this._foldCache.set(name, table)',
  '    return table',
  '  }',
  '',
  ...
  '',
  '  static CATEGORIES = {',
  '    has: (name: string): boolean => UnicodeTables.STABLE_CATEGORY_NAMES.has(name),',
  '    get: (name: string): UnicodeRangeTable | null => UnicodeTables.buildForProperty(name)',
  '  }',
  '',
  '  static SCRIPTS = {',
  '    has: (name: string): boolean =>',
  '      UnicodeTables.STABLE_SCRIPT_NAMES.has(name) || UnicodeTables.NEW_SCRIPT_NAMES.has(name),',
  '    get: (name: string): UnicodeRangeTable | null => UnicodeTables.buildForProperty(name)',
  '  }',
  '',
  '  static FOLD_CATEGORIES = {',
  '    has: (name: string): boolean => UnicodeTables.STABLE_CATEGORY_NAMES.has(name),',
  '    get: (name: string): UnicodeRangeTable | null => UnicodeTables.buildFoldOverlay(name)',
  '  }',
  '',
  '  static FOLD_SCRIPT = {',
  '    has: (name: string): boolean =>',
  ...
  '  }',
  '',
  '  static get Upper(): UnicodeRangeTable { return UnicodeTables.buildForProperty("Lu")! }',
  '}',
  '',
